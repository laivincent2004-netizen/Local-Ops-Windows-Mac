...
import { $, el, setText, setChildren, setKpi, setKpiUnit, icon, iconBtn,
  ...
  fmtUptime, fmtPct, fmtClock, localServiceUrl, executionLabel } from './core.js';
...
function watchKey(item) {
  if (item && typeof item.instanceKey === 'string' && item.instanceKey) {
    return item.instanceKey;
  }
  return item && item.pid != null ? String(item.pid) : '';
}
function findWatch(key) {
  return ((state.data && state.data.watched) || []).find(w => watchKey(w) === key);
}
  ...
  if (discoveryNeedsBaseline) {
    ...
    if (fresh.length) {
      toast(fresh.length === 1
        ? t('discovery.toastOne', {
          name: fresh[0].project || fresh[0].name || t('common.localService'),
          port: fresh[0].port,
        })
        : t('discovery.toastMany', { count: fresh.length }), 5200);
    }
  }
...
function openServiceAppModal(s) {
  ...
  openAppModal({
    ...
    execution: s.execution || (s.distro
      ? { environment: 'wsl', shell: 'posix', distro: s.distro } : null),
  });
}
...
function createServiceRow(kind) {
  ...
  cDot.setAttribute('aria-label', t(kind === 'background'
    ? 'services.statusBackground' : 'services.statusRunning'));
  /* 运行绿，后台灰 */
  ...
  mark.title = t('services.fromLaunchpad');
  ...
  const execution = el('span', 'execution-badge service-execution');
  execution.hidden = true;
  sub.append(subIcon, subText, execution);
  ...
  statusPill.textContent = t(kind === 'mine' ? 'common.running'
    : kind === 'background' ? 'services.background' : 'services.hidden');
  cStatus.append(statusPill);
  const cAct = el('span', 'c-act');
  cAct.setAttribute('role', 'cell');
  cAct.setAttribute('aria-label', t('common.actions'));
  const cmdRow = el('div', 'tr-cmd');
  cmdRow.id = 'service-command-' + (++serviceCommandId);
  cmdRow.setAttribute('role', 'note');
  cmdRow.setAttribute('aria-label', t('services.fullCommand'));
  const cmdCode = el('code');
  cmdRow.append(cmdCode);

  row.append(cDot, cTitle, cPid, cPortCell, cCwd, load.wrap, cUp, cStatus, cAct, cmdRow);
  row._kind = kind;
  row._r = { cDot, name, mark, subText, subIcon, execution, pid: cPid, port: cPort,
    cwd: cCwd, ldCpu: load.cpu, ldMem: load.mem, ldCpuFill: load.cpuFill, up: cUp,
    statusPill, cAct, cmdRow, cmdCode };
  ...
  if (kind === 'mine') {
    const bAdd = iconBtn('plus', t('services.addToLaunchpad'));
    bAdd.addEventListener('click', () => {
      const s = findSvc(key());
      openServiceAppModal(s);
    });
    const bDemote = iconBtn('download', t('services.demote'));
    bDemote.hidden = true;   // 仅被提升(promoted)的服务显示
    bDemote.addEventListener('click', () => flag('promoted', false));
    row._r.demote = bDemote;
    const bCmd = iconBtn('terminal', t('services.expandCommand'));
    ...
    row._r.pin = iconBtn('pin-off', t('services.pinToggle'));
    row._r.pin.addEventListener('click', async () => {
      const s = findSvc(key());
      if (s) flag('pinned', !s.pinned);
    });
    const bHide = iconBtn('eye-off', t('services.hide'));
    bHide.addEventListener('click', () => flag('hidden', true));
    const bKill = iconBtn('power', t('services.kill'), 'danger');
    ...
    const bPromote = iconBtn('upload', t('services.promote'));
    ...
    const bUnhide = iconBtn('eye', t('services.unhide'));
...
function updateServiceRow(row, svc) {
  ...
  r.cDot.setAttribute('aria-label', t(kind === 'background'
    ? 'services.statusBackground' : 'services.statusRunning'));
  r.mark.title = t('services.fromLaunchpad');
  setText(r.statusPill, t(kind === 'mine' ? 'common.running'
    : kind === 'background' ? 'services.background' : 'services.hidden'));
  r.cAct.setAttribute('aria-label', t('common.actions'));
  r.cmdRow.setAttribute('aria-label', t('services.fullCommand'));
  ...
  if (origin) {
    const originLabel = translateKnownText(origin.label);
    ...
    r.subIcon.title = t('services.startedBy', { origin: originLabel });
    setText(r.subText, (svc.name || '') + ' · ' + originLabel);
  } else {
    r._originIcon = null;
    r.subIcon.hidden = true;
    r.subIcon.removeAttribute('title');
  }
  const executionText = executionLabel(svc);
  ...
  if (svc.port != null) {
    ...
    r.port.title = t('services.openUrl', { url: localServiceUrl(svc, svc.port) });
    r.port.setAttribute('aria-label', t('services.openPortAria', {
      name: title,
      port: svc.port,
    }));
  } else {
  ...
  const label = (actionKey, button) => {
    if (!button) return;
    const action = t(actionKey);
    const text = t('services.actionFor', { action, name: title });
    button.title = text;
    button.setAttribute('aria-label', text);
  };
  label(svc.appId ? 'services.editLaunchpad' : 'services.addToLaunchpad', r.add);
  label('services.demote', r.demote);
  label(row.classList.contains('expanded')
    ? 'services.collapseCommand' : 'services.expandCommand', r.command);
  label(svc.pinned ? 'services.unpin' : 'services.pin', r.pin);
  label('services.hide', r.hide);
  label('services.kill', r.kill);
  label('services.promote', r.promote);
  label('services.unhide', r.unhide);
}
...
function createDiscoveryRow() {
  ...
  add.type = 'button';
  add.textContent = t('discovery.addToLaunchpad');
  ...
  ignore.type = 'button';
  ignore.textContent = t('discovery.ignoreHide');
...
function updateDiscoveryRow(row, svc) {
  const r = row._r;
  const title = svc.project || svc.name || t('common.localService');
  const process = svc.name || t('common.unknownProcess');
  setText(r.title, title);
  setText(r.status, t('discovery.justFound'));
  setText(r.meta, process +
    (svc.pid ? ' · PID ' + svc.pid : '') +
    (svc.port != null ? ' · :' + svc.port : ''));
  const detail = svc.cwd || svc.cmd || '';
  setText(r.detail, detail
    ? truncateMiddle(shortHome(detail), 72) : t('discovery.cwdUnavailable'));
  r.detail.title = detail;
  row.classList.remove('is-offline');
  setText(r.add, t('discovery.addToLaunchpad'));
  setText(r.ignore, t('discovery.ignoreHide'));
  setText(r.dismiss, t('discovery.dismiss'));
  r.add.setAttribute('aria-label', t('discovery.addAria', { name: title }));
  r.ignore.setAttribute('aria-label', t('discovery.ignoreAria', {
    name: title,
    port: svc.port,
  }));
  r.dismiss.setAttribute('aria-label', t('discovery.dismissAria', { name: title }));
}
...
function renderPortDiscovery() {
  ...
  portDiscoveryCount.setAttribute('aria-label', t('discovery.pendingAria', {
    count: items.length,
  }));
}
...
function createWatchRow() {
  ...
  row._r = { name, kw, pid: cPid, ldCpu: load.cpu, ldMem: load.mem,
    up: cUp, cAct, kill: bKill };
  return row;
}
...
function updateWatchRow(row, w) {
  ...
  const killLabel = t('services.killNamed', { name: target });
  r.kill.title = killLabel;
  r.kill.setAttribute('aria-label', killLabel);
}
...
function createChip(kw) {
  ...
  x.type = 'button';
  x.textContent = '×';
  x.title = t('watch.deleteKeyword');
  x.setAttribute('aria-label', t('watch.deleteKeywordAria', { keyword: kw }));
...
function updateChip(chip, kw) {
  chip._r.remove.title = t('watch.deleteKeyword');
  chip._r.remove.setAttribute('aria-label', t('watch.deleteKeywordAria', {
    keyword: kw,
  }));
}
...
function renderMineFilterChips(mine, onPick) {
  ...
  if (!row._btns) {
    row._btns = new Map();
    for (const [key, labelKey] of MINE_FILTERS) {
...
export function renderServices(d, firstRender) {
  ...
  setText($('#statMineSub'), t('overview.listening'));
  ...
  setText($('#statWarnSub'), t(warnCount
  ...
  reconcile(watchList, watched, watchKey, createWatchRow, updateWatchRow, firstRender);
  reconcile(watchChips, keywords, k => k, createChip, updateChip, firstRender);
  latestMine = mine;
  syncMineFilterUI();
  ...
  setText(hiddenCount, t('section.hiddenCount', { count: hid.length }));
  ...
  if (!watched.length) {
    setText(watchEmptyText, keywords.length
      ? t('table.noWatchedMatch', {
        keywords: new Intl.ListFormat(

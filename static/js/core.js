...
import { t, translateKnownText } from './i18n.js';
...
/* ---------------- 跨平台执行环境 ----------------
   execution 会随应用与监听服务返回。旧 macOS 数据可能没有该字段，
   因此所有读取都在这里补齐为 native + posix。 */
export function platformOs(platform = state.platform) {
  return String((platform && platform.os) || '').trim().toLowerCase();
}
export function isMacPlatform(platform = state.platform) {
  const os = platformOs(platform);
  return os === 'darwin' || os === 'macos' || os === 'mac';
}
export function isWindowsPlatform(platform = state.platform) {
  const os = platformOs(platform);
  return os === 'windows' || os === 'win32' || os.startsWith('win');
}
export function defaultExecution(platform = state.platform) {
  return {
    environment: 'native',
    shell: isWindowsPlatform(platform) ? 'auto' : 'posix',
    distro: null,
  };
}
export function normalizeExecution(value, platform = state.platform) {
  const raw = value && value.execution ? value.execution : value;
  const environment = raw && raw.environment === 'wsl' ? 'wsl' : 'native';
  if (environment === 'wsl') {
    const distro = raw && typeof raw.distro === 'string' && raw.distro.trim()
      ? raw.distro.trim() : null;
    return { environment: 'wsl', shell: 'posix', distro };
  }
  const requested = raw && typeof raw.shell === 'string' ? raw.shell : '';
  const shell = isWindowsPlatform(platform)
    ? (['auto', 'cmd', 'powershell'].includes(requested) ? requested : 'auto')
    : (['auto', 'cmd', 'powershell'].includes(requested) && !platform
      ? requested : 'posix');
  return { environment: 'native', shell, distro: null };
}
export function executionSignature(value, platform = state.platform) {
  const execution = normalizeExecution(value, platform);
  return [execution.environment, execution.shell, execution.distro || ''].join('|');
}
export function shellLabel(shell) {
  return ({ auto: t('platform.shell.auto'), cmd: 'CMD', powershell: 'PowerShell', posix: 'POSIX' })[shell]
    || shell || '';
}
export function executionLabel(value, platform = state.platform) {
  const raw = value && value.execution ? value.execution
    : (value && value.distro && !value.environment
      ? { environment: 'wsl', shell: 'posix', distro: value.distro }
      : value);
  const execution = normalizeExecution(raw, platform);
  if (execution.environment === 'wsl') {
    return execution.distro ? 'WSL · ' + execution.distro : 'WSL2';
  }
  /* macOS 保持原有简洁界面；Windows 才需要区分本机 Shell。 */
  if (isMacPlatform(platform)) return '';
  if (isWindowsPlatform(platform) || ['auto', 'cmd', 'powershell'].includes(execution.shell)) {
    return t('platform.nativeWindows', { shell: shellLabel(execution.shell) });
  }
  return '';
}
/* Windows / WSL 的破坏性操作必须使用不透明 instanceKey；仅在旧 macOS
   快照没有该字段时退回 pid，避免按裸 PID 误认后来复用的进程。 */
export function processIdentity(item) {
  const instanceKey = item && typeof item.instanceKey === 'string'
    ? item.instanceKey.trim() : '';
  if (instanceKey) {
    return { instanceKey };
  }
  /* 平台能力尚未加载或不可识别时也必须失败关闭。只有已经确认是 macOS
     的旧后端才允许发送兼容 pid；Windows/WSL 绝不因一次能力请求失败而
     退化成裸 PID 破坏性操作。 */
  const legacyPid = item && typeof item.pid !== 'boolean' ? Number(item.pid) : NaN;
  return isMacPlatform() && Number.isInteger(legacyPid) && legacyPid > 0
    ? { pid: legacyPid } : {};
}
...
export function fmtDuration(sec) {
  ...
  if (value < 0.1) return t('time.lessThanTenthSecond');
  if (value < 10) {
    const rounded = Math.round(value * 10) / 10;
    return t('time.seconds', {
      seconds: Number.isInteger(rounded) ? rounded.toFixed(0) : rounded.toFixed(1),
    });
  }
  const total = Math.round(value);
  if (total < 60) return t('time.seconds', { seconds: total });
  const minutes = Math.floor(total / 60), seconds = total % 60;
  if (minutes < 60) return seconds
    ? t('time.minutesSeconds', { minutes, seconds: pad2(seconds) })
    : t('time.minutes', { minutes });
  const hours = Math.floor(minutes / 60), remainMinutes = minutes % 60;
  return remainMinutes
    ? t('time.hoursMinutes', { hours, minutes: pad2(remainMinutes) })
    : t('time.hours', { hours });
}
...
async function req(method, path, body) {
  ...
  try {
    ...
    if (e.name === 'AbortError') throw new Error(t('errors.timeout'));
    throw e;
  } finally {
    clearTimeout(timer);
  }
...
export async function act(p) {
  try {
    const r = await p;
    if (r && r.ok === false) {
      toast(r.error ? translateKnownText(r.error) : t('errors.operationFailed'));
    }
    return r;
  } catch (e) {
    toast(t('errors.requestFailed', {
      message: translateKnownText(e && e.message ? e.message : String(e)),
    }));
    return null;
  }
}
...
/* 兼容旧调用方的初始值；动态界面应调用 disconnectedText()，以便语言切换后
   立即使用当前语言，而不是保留模块加载时的文案。 */
export const DISCONNECTED_TEXT = t('connection.retrying');
export const disconnectedText = () => t('connection.retrying');
...
export async function toggleTaskNotifications() {
  if (typeof Notification === 'undefined') {
    toast(t('notification.unsupported'));
    return false;
  }
  if (Notification.permission === 'default') {
    const granted = await Notification.requestPermission();
    if (granted !== 'granted') {
      toast(t('notification.noPermission'));
      return false;
    }
  }
  if (Notification.permission !== 'granted') {
    toast(t('notification.denied'));
    return false;
  }
  const enabled = taskNotificationsEnabled();
  if (enabled) {
    localStorage.removeItem(TASK_NOTIFY_KEY);
    toast(t('notification.disabled'));
  } else {
    localStorage.setItem(TASK_NOTIFY_KEY, '1');
    toast(t('notification.enabled'));
  }
  return !enabled;
}
...
export function notifyTaskCompletions(previousData, nextData) {
  ...
  for (const app of (nextData.apps || [])) {
    ...
    const suffix = duration ? t('notification.duration', { duration }) : '';
    const name = app.name || t('common.task');
    if (status === 'succeeded') {
      toast(t('notification.taskSuccess', { name, duration: suffix }), 5000);
    } else if (status === 'canceled') {
      toast(t('notification.taskCanceled', { name, duration: suffix }), 4200);
    } else {
      const result = app.lastExit.code < 0
        ? t('notification.terminated')
        : t('notification.failedExit', { code: app.lastExit.code });
      toast(t('notification.taskFailed', { name, result, duration: suffix }), 6500);
    }
    /* 页面不可见时 toast 无人看到，改用系统通知（浏览器会显示在桌面）。 */
    const notifyBody = {
      succeeded: t('notification.taskSuccess', { name: '', duration: suffix }),
      canceled: t('notification.taskCanceled', { name: '', duration: suffix }),
      failed: t('notification.taskFailed', {
        name: '',
        result: t(app.lastExit.code < 0 ? 'notification.terminated' : 'notification.failedExit', {
          code: app.lastExit.code,
        }),
        duration: suffix,
      }),
    }[status];
    systemNotify(t('notification.taskCompleteTitle', { name }),
      notifyBody || t('notification.taskEnded'));
  }
}
...
export function applyTheme() {
  ...
  setChildren(themeBtn, icon(theme === 'dark' ? 'sun' : 'moon', 15));
  themeBtn.title = theme === 'dark' ? t('theme.toLight') : t('theme.toDark');
...
function loadThemeCss(name) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onLoad = () => finish();
    const onError = () => finish(new Error(t('theme.loadFailed', { href })));
    const timer = setTimeout(() => finish(new Error(t('theme.loadTimeout'))), 8000);
...
export function applyUiTheme(name, persist = false) {
  ...
  const queued = themeChangeQueue.catch(() => false).then(async () => {
    ...
    if (!/^[a-z0-9][a-z0-9_-]{0,63}$/i.test(name)
      ...
      if (persist) toast(t('theme.notFound'));
      return false;
    }
    ...
    try {
      ...
      if (persist) {
        ...
        if (!result || result.ok === false) {
          throw new Error(result && result.error
            ? translateKnownText(result.error) : t('theme.saveFailed'));
        }
      }
      return true;
    } catch (e) {
      ...
      if (persist) {
        toast(t('theme.switchFailed', { message: translateKnownText(e.message) }));
      }
      return false;
    }
  });

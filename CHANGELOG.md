...
- 新增 Windows 10 22H2 / Windows 11 x64 安装版：PyInstaller `onedir + windowed` 托盘宿主、默认浏览器界面、当前用户单实例激活，以及无需另行安装 Python 的 Inno Setup per-user 安装包。
- 新增 Windows 原生运行环境与 Shell 选择：`auto`、CMD 和 Windows PowerShell；项目识别、脚本命令、路径选择和进程溯源按平台生成正确结果。
- 新增 WSL2 x86_64 完整支持：仅并行扫描已运行发行版、Linux/UNC/`/mnt` 路径映射、localhost/发行版地址端点，以及无需发行版 Python/`ps`/`lsof` 的静态 Rust/musl `/proc` helper。
- 新增独立版本化 Windows supervisor：使用不启用 `KILL_ON_JOB_CLOSE` 的命名 Job Object、只当前 SID 可访问的命名管道、HMAC 控制和持久化运行元数据，使 Windows/WSL2 服务在托盘或 HTTP 服务退出后继续运行并可重新识别。
- 新增 `GET /api/platform`，返回操作系统、架构、可用 Shell、打包状态以及已安装 WSL 发行版的版本/运行/可用状态。
- 新增 Windows 发行流水线：Linux CI 构建并验证静态 helper，Windows CI 运行 Python/JavaScript 测试、打包托盘应用/独立 supervisor/Inno Setup 安装器，并输出 SHA-256、发行清单、SPDX 2.3 SBOM、完整依赖清单和未签名 SmartScreen 说明。
...
- 顶栏新增 `EN | 中` 语言滑动开关，内建界面可即时切换为完整英文或中文并记住浏览器偏好；用户名称、命令、路径和日志保持原样。

### Changed

- 配置升级为 `schemaVersion=2`，应用增加 `execution {environment,shell,distro}`。macOS v1 卡片幂等迁移为 `native + posix`；Windows 从 `%LOCALAPPDATA%\总控台` 创建全新配置，不扫描或导入 macOS 数据。
- `/api/state` 中应用和服务增加 `execution`；`instanceKey` 升级为包含环境、发行版、boot/create-time、PID 与命令/目录身份的有签名不透明标识。Windows/WSL 进程认领与破坏性操作不再接受裸 PID。
- Windows 默认将配置、图标、日志、运行元数据和版本化 supervisor 存入 `%LOCALAPPDATA%\总控台`；安装到 `%LOCALAPPDATA%\Programs\总控台`，升级和卸载只停止托盘/HTTP，保留已运行应用与用户数据；应用退出后清理已无运行元数据引用的旧 supervisor，并在后续启动时兜底。
- 普通停止改为两阶段协议：Windows 先发 `CTRL_BREAK_EVENT`，WSL 先发 SIGTERM；超时保留身份并返回 `requiresForce`，只在用户明确二次确认后才强制结束已验证 Job/session。
...
- 修复 Windows 安装版后台刷新 WSL 发行版与 helper 状态时反复闪出命令提示符窗口；所有生产 WSL 子进程现以无控制台窗口方式启动，测试注入与 macOS 命令路径保持不变。
...
- Windows 进程操作改为校验当前 SID、PID 创建时间、端口、cwd 和命令身份；数据、元数据和命名管道 DACL 仅允许当前 SID 与 SYSTEM。不能完整证明身份时失败，不退化为按端口或裸 PID 杀进程。
- WSL helper 仅返回当前 Linux UID，session 使用 0600 Unix socket、`SO_PEERCRED`、token hash、boot ID、start ticks 与 PGID；外部进程用 pidfd 和完整 UID/boot/start/cwd/cmd 身份发信号，防止 PID 重用。
- WSL favicon 拉取限定为刚发现的发行版地址+端口，禁止代理和跨端点重定向；发行版名称和 Windows/WSL 路径均执行严格校验。

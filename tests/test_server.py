...
POSIX_ONLY = unittest.skipIf(
    server.IS_WINDOWS, "macOS/POSIX process semantics are tested on macOS CI")
...
class ParsingTests(unittest.TestCase):
    ...
    def test_http_log_message_preserves_writable_stderr(self):
        handler = object.__new__(server.Handler)
        handler.path = "/api/health"
        handler.client_address = ("127.0.0.1", 12345)
        stream = io.StringIO()
        with mock.patch.object(server.sys, "stderr", stream):
            handler.log_message('"%s" %s', "GET /api/health", 200)
        self.assertEqual(
            stream.getvalue(), '127.0.0.1 - "GET /api/health" 200\n'
        )

    def test_health_response_survives_windowed_none_stderr(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            thread.start()
            try:
                with mock.patch.object(server.sys, "stderr", None), self.assertLogs(
                    server.LOG, level="INFO"
                ) as captured:
                    conn = http.client.HTTPConnection(server.HOST, port, timeout=4)
                    try:
...
class OriginAttributionTests(unittest.TestCase):
    ...
    def test_quoted_windows_executable_paths_are_not_split_at_spaces(self):
        cases = (
            (r'"C:\Program Files\WindowsApps\Microsoft.WindowsTerminal.exe" -Embedding',
             {"label": "Windows Terminal", "icon": "terminal"}),
            (r'"C:\Program Files\Microsoft VS Code\Code.exe" --type=utility',
             {"label": "VS Code", "icon": "code"}),
            (r'C:\Windows\System32\cmd.exe /d /s /c npm run dev',
             {"label": "CMD", "icon": "terminal"}),
        )
        for parent, expected in cases:
            with self.subTest(parent=parent):
                table = self.table((100, 90, "node server.js"),
                                   (90, 1, parent))
                self.assertEqual(server.attribute_origin(100, table), expected)
...
class ProjectDetectionTests(unittest.TestCase):
    ...
    def test_detects_django_and_static_site_fallback(self):
        ...
        python_command = "py -3" if server.IS_WINDOWS else "python3"
        self.assertEqual(
            django["candidates"][0]["command"],
    ...
    def test_windows_compose_go_rust_and_script_matrix(self):
        execution = {"environment": "native", "shell": "auto", "distro": None}
        cases = (
            ("compose.yaml", 'services:\n  web:\n    ports: ["8123:80"]\n',
             "docker compose up", 8123),
            ("go.mod", "module example.test/app\n", "go run .", None),
            ("Cargo.toml", '[package]\nname="demo"\nversion="0.1.0"\n',
             "cargo run", None),
            ("start.ps1", "Write-Output ok\n",
             "powershell.exe -NoLogo -NoProfile -File .\\start.ps1", None),
            ("start.cmd", "@echo off\r\necho ok\r\n", ".\\start.cmd", None),
        )
        for filename, contents, command, port in cases:
            with self.subTest(filename=filename), tempfile.TemporaryDirectory() as td, \
                    mock.patch.object(server, "IS_WINDOWS", True):
                project = os.path.join(td, "项目 with spaces")
                os.mkdir(project)
                with open(os.path.join(project, filename), "w",
                          encoding="utf-8") as output:
                    output.write(contents)
                result, error = server.detect_project(project, execution)
                self.assertIsNone(error)
                candidate = next(
                    item for item in result["candidates"]
                    if item["command"] == command)
                self.assertEqual(candidate["port"], port)
                self.assertEqual(result["cwd"], os.path.abspath(project))
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            legacy["apps"] = [{
                **server.Config.APP_DEFAULT,
                "id": "deadbeef", "name": "旧服务", "command": "echo ok",
            }]
            with open(path, "w", encoding="utf-8") as f:
                json.dump(legacy, f)

            # Legacy repository data is a macOS format. Windows deliberately
            # starts fresh, while this unit test keeps the macOS v0->v2
            # migration contract executable on either CI host.
...
class ProcessIdentityTests(unittest.TestCase):
    ...
    def test_task_exit_records_duration_and_unique_run_time(self):
        with tempfile.TemporaryDirectory() as td, \
            ...
            app = {**server.Config.APP_DEFAULT, "id": "deadbeef",
                   ...
                   "execution": server.normalize_execution(None)[0]}
            with open(path, "w", encoding="utf-8") as f:
    ...
    def test_started_runtime_persist_error_enters_verified_rollback(self):
        proc = mock.Mock(pid=4321)
        proc.runtime_identity = {"type": "supervisor", "runId": "run-1"}
        cfg = mock.Mock()
        with mock.patch.object(
                server, "persist_started_app",
                side_effect=OSError("disk full")) as persist, \
                mock.patch.object(
                    server, "cancel_unpersisted_runtime",
                    return_value=(True, None)) as cancel:
            committed, error, retained = server.persist_started_app_or_rollback(
                cfg, "deadbeef", proc, 4321, "runtime-token")
        ...
        persist.assert_called_once_with(
            cfg, "deadbeef", proc, 4321, "runtime-token")
        cancel.assert_called_once_with(proc, 4321, "runtime-token")

    def test_failed_rollback_retains_token_and_identity_in_memory(self):
        with tempfile.TemporaryDirectory() as td:
            path = os.path.join(td, "config.json")
            app = {**server.Config.APP_DEFAULT, "id": "deadbeef",
                   "name": "服务", "command": "serve",
                   "execution": server.normalize_execution(None)[0]}
            with open(path, "w", encoding="utf-8") as f:
                json.dump({**server.Config.DEFAULT, "apps": [app]}, f)
            cfg = server.Config(path)
            ...
            with mock.patch.object(
                    ...
                    mock.patch.object(
                        server, "cancel_unpersisted_runtime",
                        return_value=(False, "force-stop failed")), \
                    mock.patch.object(server, "watch_app_exit") as watch:
                committed, error, retained = server.persist_started_app_or_rollback(
                    cfg, "deadbeef", proc, 4321, "runtime-token")

            stored = cfg.snapshot()["apps"][0]
            health = cfg.health_info()

        self.assertFalse(committed)
        self.assertTrue(retained)
        self.assertIn("请勿退出", error)
        self.assertEqual(stored["runToken"], "runtime-token")
        self.assertEqual(stored["processIdentity"], proc.runtime_identity)
        self.assertEqual(stored["lastPid"], 4321)
        self.assertTrue(any("仅保留在当前总控台内存" in issue
                            for issue in health["issues"]))
        watch.assert_called_once()

    def test_unpersisted_supervisor_cleanup_rejects_unconfirmed_stop(self):
        token = "t" * 43
        proc = server.SupervisorRuntimeHandle(
            "runtime.json",
            {"supervisorPid": 111, "childPid": 222, "runId": "run-1"},
            token,
        )
        with mock.patch.object(server, "IS_WINDOWS", True), \
                mock.patch(
                    ...
                    return_value={
                        ...
                        "error": "authenticated force-stop failed",
                    }) as stop:
            cleaned, error = server.cancel_unpersisted_runtime(
                proc, 111, token)

        self.assertFalse(cleaned)
        self.assertIn("force-stop failed", error)
        stop.assert_called_once_with(
            "runtime.json", token, force=True,
            timeout=server.APP_STOP_TIMEOUT_SEC + 3.0)


class LaunchEnvironmentTests(unittest.TestCase):
    @unittest.skipUnless(os.name == "nt", "Windows PATH semantics")
    def test_windows_launch_path_deduplicates_case_insensitively(self):
        home = os.path.expanduser("~")
        local_bin = os.path.join(home, ".local", "bin")
        with mock.patch.object(server, "IS_WINDOWS", True):
            env = server.build_launch_env(
                "secret", {"PATH": local_bin.upper() + os.pathsep + local_bin}
            )
        matching = [
            path for path in env["PATH"].split(os.pathsep)
            if os.path.normcase(os.path.normpath(path))
            == os.path.normcase(os.path.normpath(local_bin))
        ]
...
class StateTests(unittest.TestCase):
    ...
    def test_windows_watched_snapshot_is_nonblocking_and_deduplicated(self):
        entered = threading.Event()
        release = threading.Event()
        expected = [{"pid": 10, "name": "ffmpeg"}]

        def slow_scan(keywords):
            entered.set()
            self.assertTrue(release.wait(3.0))
            return expected

        with server._WINDOWS_WATCH_LOCK:
            original = (
                server._WINDOWS_WATCH_THREAD,
                server._WINDOWS_WATCH_DESIRED,
                server._WINDOWS_WATCH_CACHE_KEY,
                server._WINDOWS_WATCH_CACHE,
                server._WINDOWS_WATCH_LAST_SCAN,
            )
            server._WINDOWS_WATCH_THREAD = None
            server._WINDOWS_WATCH_DESIRED = ()
            server._WINDOWS_WATCH_CACHE_KEY = ()
            server._WINDOWS_WATCH_CACHE = []
            server._WINDOWS_WATCH_LAST_SCAN = 0.0
        worker = None
        try:
            with mock.patch.object(
                    server, "build_watched", side_effect=slow_scan) as scan:
                self.assertEqual(
                    server.windows_watched_snapshot(["ffmpeg"]), [])
                self.assertTrue(entered.wait(1.0))
                worker = server._WINDOWS_WATCH_THREAD
                self.assertEqual(
                    server.windows_watched_snapshot(["ffmpeg", "ffmpeg"]), [])
                self.assertIs(server._WINDOWS_WATCH_THREAD, worker)
                release.set()
                worker.join(3.0)
                self.assertFalse(worker.is_alive())
                self.assertEqual(
                    server.windows_watched_snapshot(["ffmpeg"]), expected)
                scan.assert_called_once_with(("ffmpeg",))
        finally:
            release.set()

...
class ReleaseFixtureTests(unittest.TestCase):
    ...
    def test_symlinked_required_source_is_rejected(self):
        ...
        try:
            ...
            if os.name == "nt" and getattr(exc, "winerror", None) == 1314:
                self.skipTest("Windows 未启用创建符号链接的权限")
            raise
    ...
    def test_archive_is_reproducible_and_metadata_is_normalized(self):
        ...
        if os.name != "nt":
            # Windows does not expose the Unix write bits applied to the host
            # zip file. Member modes below remain portable and authoritative.
...
class ProjectReleaseManifestTests(unittest.TestCase):
    def test_untracked_user_plan_is_not_a_release_input(self):
        self.assertNotIn("PLAN.md", release.INCLUDE)
        plan = release.ROOT / "PLAN.md"
        original_exists = Path.exists

        def exists_without_plan(path):
            if path == plan:
                return False
            return original_exists(path)

        with mock.patch.object(
            Path, "exists", autospec=True, side_effect=exists_without_plan
        ):
            names = {
    ...
    def test_cross_platform_runtime_and_build_sources_are_in_payload(self):
        names = {
            path.relative_to(release.ROOT).as_posix()
            for path in release.iter_release_files()
        }

...
class HttpSecurityTests(unittest.TestCase):
    ...
    def _session_cookie(self):
        # The lightweight endpoint is sufficient to establish the browser
        # session and intentionally avoids real ps/lsof/WSL probes in an HTTP
        # security unit test.
...
class DeliveryMetadataTests(unittest.TestCase):
    ...
    def test_state_exposes_version_schema_and_component_degradation(self):
        with mock.patch.object(server, "build_services",
                ...
                mock.patch.object(
                    server,
                    "windows_watched_snapshot" if server.IS_WINDOWS
                    else "build_watched",
                    return_value=[]), \
...
class AppConfigurationTests(unittest.TestCase):
    ...
    def test_multiple_launch_profiles_may_share_a_configured_port(self):
        ...
        self.assertEqual(status, 409)
        self.assertIn("PID 999 占用", body["error"])
...
class StaticFileServingTests(unittest.TestCase):
    ...
    @POSIX_ONLY
    def test_symlink_inside_icons_cannot_escape_to_outside(self):
        with tempfile.TemporaryDirectory() as td:
            outside = os.path.join(td, "secret.png")
            with open(outside, "wb") as f:
                f.write(b"not-an-icon-secret")
            icons = os.path.join(td, "icons")
            os.mkdir(icons)
            os.symlink(outside, os.path.join(icons, "leak.png"))
            with mock.patch.object(server, "ICONS_DIR", icons):
                status, body, _ = self.h.request("GET", "/icons/leak.png")
            self.assertEqual(status, 404)
            self.assertNotIn(
                b"not-an-icon-secret", body if isinstance(body, bytes) else b""
            )

    @POSIX_ONLY
    def test_private_icon_write_refuses_existing_symlink_target(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            icons = os.path.join(td, "icons")
            os.mkdir(icons)
            link = os.path.join(icons, "app.png")
            os.symlink(outside, link)
            with self.assertRaisesRegex(OSError, "符号链接"):
                server.write_private_bytes(link, b"replace-me")
            with open(outside, "rb") as source:
                self.assertEqual(source.read(), b"keep-me")
...
class StateCacheTests(unittest.TestCase):
    ...
    def test_config_update_during_build_cannot_deadlock_or_publish_old_cache(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            def fake_build(snapshot, port, health=None):
                theme = snapshot["uiTheme"]
                builds.append(theme)
                if len(builds) == 1:
                    build_entered.set()
                    if not release_build.wait(2):
                        raise AssertionError("build release timed out")
                return {"theme": theme, "port": port}

            def read_state():
                try:
                    results.append(server.get_state_snapshot(cfg, 9600))
            ...
            def update_config():
                try:
                    cfg.update(
                        lambda data: data.__setitem__("uiTheme", "custom"))
                except Exception as exc:  # pragma: no cover - asserted below
                    failures.append(exc)
                finally:
                    update_done.set()
            ...
            with mock.patch.object(server, "build_state",
                ...
                update_thread.start()
                update_completed_while_old_build_waited = update_done.wait(0.5)
                release_build.set()
                state_thread.join(2)
                update_thread.join(2)

                self.assertTrue(update_completed_while_old_build_waited)
                self.assertFalse(state_thread.is_alive())
                self.assertFalse(update_thread.is_alive())
                self.assertEqual(failures, [])
                self.assertEqual(results, [{"theme": "ops", "port": 9600}])
                # The concurrent invalidation rejected the old result.
                self.assertIsNone(server._state_cache["state"])

                fresh = server.get_state_snapshot(cfg, 9600)
                reused = server.get_state_snapshot(cfg, 9600)

            self.assertEqual(builds, ["ops", "custom"])
            self.assertEqual(fresh, {"theme": "custom", "port": 9600})
            self.assertIs(reused, fresh)

    def test_concurrent_cache_misses_share_one_build_and_one_result(self):
        build_entered = threading.Event()
        release_build = threading.Event()
        results = []
        failures = []
        calls = []
        cfg = mock.Mock()
        cfg.snapshot.return_value = dict(server.Config.DEFAULT)
        cfg.health_info.return_value = {}

        def fake_build(snapshot, port, health=None):
            calls.append((snapshot, port))
            build_entered.set()
            if not release_build.wait(2):
                raise AssertionError("build release timed out")
        ...
        self.assertEqual(failures, [])
        self.assertEqual(len(calls), 1)
        self.assertEqual(results, [{"built": 1}, {"built": 1}])

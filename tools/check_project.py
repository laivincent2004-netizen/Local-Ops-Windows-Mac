...
def check_required_files() -> str:
    required = (
        ...
        "supervisor_windows.py",
        "console_platform/__init__.py",
        "console_platform/windows.py",
        "console_platform/wsl_host.py",
        "desktop/windows_host.py",
        "packaging/windows/build.ps1",
        "packaging/windows/console.spec",
        "packaging/windows/supervisor.spec",
        "packaging/windows/installer.iss",
        "wsl_helper/Cargo.toml",
        "wsl_helper/Cargo.lock",
        "wsl_helper/rust-toolchain.toml",
        "wsl_helper/src/main.rs",
        "start.command",
...
def check_python_syntax() -> str:
    paths = {
        ROOT / "server.py",
        ROOT / "supervisor.py",
        ROOT / "supervisor_client.py",
        ROOT / "supervisor_windows.py",
    }
    for directory in ("tools", "tests", "console_platform", "desktop"):
...
def check_shell_and_plist() -> str:
    ...
    if os.name == "nt":
        for path in shell_files:
            require(path.is_file(), f"{path.relative_to(ROOT)} 不存在")
            raw = path.read_bytes()
            require(raw.startswith(b"#!/bin/bash"),
                    f"{path.relative_to(ROOT)} 缺少 bash shebang")
            require(b"\x00" not in raw and b"\r\n" not in raw,
                    f"{path.relative_to(ROOT)} 必须保持 UTF-8/LF 文本")
        try:
            with INFO_PLIST.open("rb") as stream:
                plistlib.load(stream)
        except (OSError, plistlib.InvalidFileException) as exc:
            raise CheckError(f"Info.plist 无效: {exc}") from exc
        return "2 个 macOS 启动脚本静态检查 + Info.plist"
    for path in shell_files:
...
def parse_locked_requirements(
    text: str, *, filename: str, require_hashes: bool = False
) -> list[str]:
    """Return validated logical requirement lines from an exact lock file."""
    ...
    for raw_line in text.splitlines():
        ...
        if not continued:
            ...
            pending = ""
    require(not pending, f"{filename} 存在未结束的续行")
    require(bool(logical_lines), f"{filename} 为空")
    ...
    for line in logical_lines:
        ...
        pinned = bool(
            parts
            and re.fullmatch(
                r"[A-Za-z0-9_.-]+==[A-Za-z0-9_.+!-]+", parts[0]
            )
        )
        hashes = parts[1:]
        if not pinned or any(
            ...
            invalid.append(line)
        elif require_hashes and not hashes:
            unhashed.append(line)
    require(not invalid,
            f"{filename} 依赖必须精确锁定: " + ", ".join(invalid))
    require(not unhashed,
            f"{filename} 依赖缺少 SHA-256 锁: " + ", ".join(unhashed))
...
def check_dev_requirements() -> str:
    ...
    for filename in ("requirements-dev.txt", "requirements-windows.txt"):
        path = ROOT / filename
        lines = parse_locked_requirements(
            path.read_text(encoding="utf-8"),
            filename=filename,
            require_hashes=filename == "requirements-windows.txt",
        )
        total += len(lines)
    return f"{total} 个锁定依赖（开发 + Windows）"
...
def check_javascript_tests() -> str:
    ...
    # Node 24 changed the default test reporter from TAP comments
    # (``# pass 7``) to the spec reporter (``ℹ pass 7``).  Accept both
    # machine-stable summary forms so a successful suite cannot make the
    # project gate fail merely because the installed Node major changed.
    passed_match = re.search(r"^(?:#|ℹ)\s+pass\s+(\d+)\s*$", output, re.M)
    failed_match = re.search(r"^(?:#|ℹ)\s+fail\s+(\d+)\s*$", output, re.M)
    require(passed_match is not None and failed_match is not None,
            "无法确认 node --test 结果")
    passed = int(passed_match.group(1))
    failed = int(failed_match.group(1))
    require(passed > 0, "JavaScript 测试发现结果为 0")
...
def main() -> int:
    # Redirected Windows consoles commonly default to cp1252 even though every
    # project diagnostic is Chinese. Keep PowerShell and CI output deterministic
    # without requiring callers to set PYTHONUTF8.
